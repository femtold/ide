 ...
 * 
 */
public class PreferenceEditor extends PreferencePage implements
		...
		for (HighlightingColorListItem hcli : fHighlightingColorList) {
			if (hcli.hasChanged()) {
				data = TextAttributeConverter.textAttributesToPreferenceData(
						hcli.getItemColor(), hcli.getItemBold(),
						hcli.getItemItalic());
		...
		if (changes) {
			...
			try {
				InstanceScope.INSTANCE
						.getNode(IDConstants.PREFERENCE_EDITOR_ID).flush();
