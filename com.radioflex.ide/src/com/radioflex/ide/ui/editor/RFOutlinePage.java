...
import com.radioflex.ide.ui.editor.RFOutlineParser.LocatedData;

public class RFOutlinePage extends ContentOutlinePage {

	private RadioFlexEditor editor;

	private IEditorInput input;

	public RFOutlinePage(RadioFlexEditor editor) {
		this.editor = editor;

		// FT.print("in RFOutlinePage Constructor");
	...
	public void setInput(IEditorInput input) {
		// FT.print("start in RFOutlinePage setInput");
		...
		if ((viewer != null) && (viewer.getContentProvider() != null)) {
			...
			editor.getSite().getShell().getDisplay().asyncExec(new Runnable() {
				public void run() {
					...
					// FT.print(" in RFOutlinePage setInput: new Runnable innerclass");
					...
					viewer.setInput(RFOutlinePage.this.input);
					viewer.refresh();
					viewer.expandAll();
		...
		// FT.print("end in RFOutlinePage setInput");
	}
	...
	private TreeNode getSelectedTreeModelObject(TreeViewer viewer) {
		...
		// FT.print("start in RFOutlinePage getSelectedTreeObject");
		...
		if (!selection.isEmpty()) {
			if (selection instanceof IStructuredSelection) {
				...
				if (object instanceof TreeNode) {
					...
					// FT.print("end in RFOutlinePage getSelectedTreeObject 1"+
		...
		// FT.print("end in RFOutlinePage getSelectedTreeObject 2"+
		// "return null;");
	...
	private void selectTreeModelObject(TreeViewer viewer, TreeNode treenode) {
		...
		// FT.print("start in RFOutlinePage selectTreeObject");
		...
		// FT.print("end in RFOutlinePage selectTreeObject");
	...
	public void createControl(Composite parent) {
		...
		// FT.print("start (after super.createControl) in RFOutlinePage createControl");
		...
		// FT.print("end in RFOutlinePage createControl");
	}
	...
	public void selectionChanged(SelectionChangedEvent event) {
		...
		// FT.print("start (after super.selectionChanged) in RFOutlinePage selectionChanged");
		...
		// FT.print("end in RFOutlinePage selectionChanged");
	...
	private class REOutlineContentProvider implements ITreeContentProvider {
		private IEditorInput input;
		RFOutlineParser parser;

		public REOutlineContentProvider() {
			parser = new RFOutlineParser();
		}
		...
		public Object[] getElements(Object inputElement) {
			if (inputElement == this.input) {
				ArrayList<TreeNode> all = parser.getOutlineTree();
				for (int i = 0; i < all.size(); i++) {
					if (all.get(i).getChildren().length == 0) {
						all.remove(i);
					}
				}
		...
		public void inputChanged(Viewer viewer, Object oldInput, Object newInput) {
			...
			if (newInput instanceof IEditorInput) {
				...
				IDocument document = RFOutlinePage.this.editor
